...
ui <- fluidPage(
  ...
  sidebarPanel(width = 3,style = "background:rgba(135, 206, 235, 0.3)",
               ...
               h5(div(HTML("*<em>Note: this calculator is for male only.</em>")))),            
  ...
  mainPanel(
    tabsetPanel(
      ...
      wellPanel(style = "background: transparent;border-color:transparent",
        conditionalPanel(
          condition = "output.weight > 0 && output.weight <= 1000 && output.abdomen > 0 && output.abdomen <=500 && output.wrist > 0 && output.wrist <= 80 && output.esti > 0 && output.esti < 100",
          h5(textOutput("value")),
        ...
        conditionalPanel(
          ...
          h5("1. You imported an non-numeric or empty value in either the 3 input boxes."),
          h5("2. You imported values less or equal to 0, or a value too high for human beings in either the 3 input boxes."),
          h5("3. The estimated body fat can't be real for human beings.(around 100% or apprximate 0%)")
          ))),
